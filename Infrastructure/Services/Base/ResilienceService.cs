...
namespace Infrastructure.Services.Base
{
    public class ResilienceService<T> : IResilienceService<T>
    {
        ...
        {
            // Instead of using a complex wrapper, create a new service instance with correct type
        ...
        public static SafeExecuteOptions<T> ForQuickOperation<T>(TimeSpan? timeout = null)
        {
            ...
            return new SafeExecuteOptions<T>()
                ...
                .WithRetry(2); // Fewer retries for quick operations
        }
            ...
            SafeExecuteOptions<TResult>? options = null)
        {
            ...
            try
            {
                ...
                // Enhanced resilience execution using Microsoft.Extensions.Resilience
                var result = await ExecuteWithResilience(work, options, enrichedScope);

                stopwatch.Stop();
                var duration = stopwatch.Elapsed; // Keep as TimeSpan
                var durationSeconds = duration.TotalSeconds; // Extract double for metrics

                // Performance threshold checking - Fixed type comparison
                if (options?.PerformanceThreshold.HasValue == true && duration > options.PerformanceThreshold.Value)
                {
                    options.OnSlowOperation?.Invoke(duration); // Pass TimeSpan to callback
            ...
            catch (Exception ex)
            {
                ...
                // Error metrics and comprehensive logging
                ErrorCounter.Add(1,
                    new KeyValuePair<string, object?>("operation", scope.OperationName),
                    new KeyValuePair<string, object?>("entity_type", typeof(TResult).Name),
            ...
            Scope scope)
        {
            try
            {
                // If no options provided, execute without resilience
                ...
                // Use the pipeline from extensions (WithHttpResilience, WithDatabaseResilience, etc.)
        ...
        // Non-generic SafeExecute method
        ...
        // Non-generic resilience execution
