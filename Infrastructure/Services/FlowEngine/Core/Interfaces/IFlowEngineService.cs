...
namespace Infrastructure.Services.FlowEngine.Core.Interfaces
{
    ...
    {
        Task<FlowExecutionResult> StartAsync<TFlow>(Dictionary<string, object>? initialData = null, string userId = null, string correlationId = null, CancellationToken cancellationToken = default) where TFlow : FlowDefinition;
        Task<Flow?> GetFlowById(Guid flowId);
        FlowStatus? GetStatus(Guid flowId);
        Task<FlowExecutionResult> ResumeRuntimeAsync(Guid flowId, CancellationToken cancellationToken = default);
