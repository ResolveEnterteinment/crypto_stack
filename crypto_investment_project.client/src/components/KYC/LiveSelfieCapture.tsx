...
import { DocumentCaptureData, ImageData, SelfieCaptureData } from '../../types/kyc';
...
interface LiveSelfieCaptureProps {
    onCapture: (captureData: SelfieCaptureData) => Promise<void>;
}

const LiveSelfieCapture: React.FC<LiveSelfieCaptureProps> = ({
    onCapture
}) => {
    ...
    const [captureResult, setCaptureResult] = useState<SelfieCaptureData | null>(null);
    ...
    const captureSelfie = useCallback(async () => {
        ...
        try {
            ...
            const captureData: SelfieCaptureData = {
                isLive: livenessDetection.isLive,
                imageData: imageData,
                captureMetadata: {
                    timestamp: Date.now(),
    ...
    const getCardTitle = () => {
        return `Live selfie capture`;
    };

    const getInstructions = () => {
        return 'Position your head in the frame and click capture';
    };
